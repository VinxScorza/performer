...
- `16-step Editing Mode`: Experimental external controller workflow for `Launch Control XL` / `BeatStep Pro` style 16-knob/16-pad mappings on `Note` tracks. Knobs control `Note` values and pads toggle `Gate` states for the current visible 16-step bank. Armed mode forces playback/edit scope to that 16-step window; `Prev`/`Next` moves across longer loops bank-by-bank and disarm restores the previous loop range. Pad/function LED feedback is available; knob-row LED behavior remains device-local. `BeatStep Pro` remains code-supported but still requires broader on-hardware validation; maintainer-side hardware tests so far are on `Launch Control XL` only.
